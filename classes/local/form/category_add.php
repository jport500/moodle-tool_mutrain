 ...
 */
final class category_add extends \tool_mulib\local\ajax_form {
    #[\Override]
    protected function definition() {
        ...
        $mform->addElement('text', 'categoryname', get_string('categoryname', 'tool_mutrain'), 'maxlength="64" size="30"');
        ...
        $this->add_action_buttons(true, get_string('categoryruleadd', 'tool_mutrain'));
    }
    ...
    public function validation($data, $files) {
        ...
        } else if ($DB->record_exists('tool_mutrain_framework_category', [
            'frameworkid' => $framework->id,
            'categoryname' => trim($data['categoryname']),
        ])) {
            $errors['categoryname'] = get_string('categoryruleduplicate', 'tool_mutrain');
        }

        $mincredits = str_replace(',', '.', $data['mincredits']);
        if (!is_numeric($mincredits) || (float)$mincredits <= 0) {
            $errors['mincredits'] = get_string('categorymincreditsinvalid', 'tool_mutrain');
        }
