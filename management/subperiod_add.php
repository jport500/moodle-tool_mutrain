...
$currenturl = new core\url('/admin/tool/mutrain/management/subperiod_add.php', ['id' => $frameworkid]);
...
$form = new \tool_mutrain\local\form\subperiod_add(null, ['framework' => $framework, 'context' => $context]);

if ($form->is_cancelled()) {
    $form->ajax_form_cancelled($returnurl);
} else if ($data = $form->get_data()) {
    $rc = (float)str_replace(',', '.', (string)$data->requiredcredits);
    \tool_mutrain\api::add_subperiod(
        (int)$framework->id,
        (string)$data->name,
        (string)$data->mode,
        (int)($data->offsetdays ?? 0),
        (int)($data->lengthdays ?? 365),
        (int)($data->startdate ?? 0),
        (int)($data->enddate ?? 0),
        $rc,
        0
    );
